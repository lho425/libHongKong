...
static clockid_t find_monotonic_clock(void)
{
	...
	if (r == 0) {
		...
		usbi_dbg("monotonic clock doesn't work, errno %d", errid);
...
static int __open_sysfs_attr(struct libusb_device *dev, const char *attr)
{
	...
	if (fd < 0) {
		usbi_err(DEVICE_CTX(dev),
			"open %s failed ret=%d errno=%d", filename, fd, errid);
	...
	unsigned char *buffer)
{
	...
	close(fd);
	if (r < 0) {
		usbi_err(DEVICE_CTX(dev), "read failed, ret=%d errno=%d", fd, errid);
		return LIBUSB_ERROR_IO;
...
static int sysfs_get_active_config(struct libusb_device *dev, int *config)
{
	...
	if (r < 0) {
		usbi_err(DEVICE_CTX(dev), 
			"read bConfigurationValue failed ret=%d errno=%d", r, errid);
		return LIBUSB_ERROR_IO;
	} else if (r == 0) {
	...
	int host_endian)
{
	...
	r = read(fd, tmp, sizeof(tmp));
	if (r < 0) {
		usbi_err(ctx, "read failed ret=%d errno=%d", r, errid);
		return LIBUSB_ERROR_IO;
	...
	if (off < 0) {
		usbi_err(ctx, "seek failed ret=%d errno=%d", off, errid);
		return LIBUSB_ERROR_IO;
	}
	...
	unsigned char *buffer, size_t len)
{
	...
	if (off < 0) {
		usbi_err(DEVICE_CTX(dev), "seek failed, ret=%d errno=%d", off, errid);
		close(fd);
		return LIBUSB_ERROR_IO;
	}
...
static int usbfs_get_active_config(struct libusb_device *dev, int fd)
{
	...
	if (r < 0) {
		...
		usbi_warn(DEVICE_CTX(dev),
			"get_configuration failed ret=%d errno=%d", r, errid);
		return LIBUSB_ERROR_IO;
	}
	...
	uint8_t devaddr, const char *sysfs_dir)
{
	...
	if (fd < 0) {
		usbi_err(DEVICE_CTX(dev), "open failed, ret=%d errno=%d", fd, errid);
		return LIBUSB_ERROR_IO;
	}
	...
	if (r < 0) {
		usbi_err(DEVICE_CTX(dev),
			"read descriptor failed ret=%d errno=%d", fd, errid);
		free(dev_buf);
		close(fd);
		return LIBUSB_ERROR_IO;
	} else if (r < DEVICE_DESC_LENGTH) {
	...
	struct discovered_devs **_discdevs, uint8_t busnum)
{
	...
	dir = opendir(dirpath);
	if (!dir) {
		usbi_err(ctx, "opendir '%s' failed, errno=%d", dirpath, errid);
	...
	struct discovered_devs **_discdevs)
{
	...
	if (!buses) {
		usbi_err(ctx, "opendir buses failed errno=%d", errid);
		return LIBUSB_ERROR_IO;
	}
	...
	int *usbfs_fallback)
{
	...
	if (!fd) {
		...
		usbi_err(ctx, "open busnum failed, errno=%d", errid);
		return LIBUSB_ERROR_IO;
	}
	...
	fclose(fd);
	if (r != 1) {
		usbi_err(ctx, "fscanf busnum returned %d, errno=%d", r, errid);
		return LIBUSB_ERROR_IO;
	}
	...
	fd = fopen(filename, "r");
	if (!fd) {
		usbi_err(ctx, "open devnum failed, errno=%d", errid);
		return LIBUSB_ERROR_IO;
	}
	...
	fclose(fd);
	if (r != 1) {
		usbi_err(ctx, "fscanf devnum returned %d, errno=%d", r, errid);
		return LIBUSB_ERROR_IO;
	}
	...
	struct discovered_devs **_discdevs, int *usbfs_fallback)
{
	...
	if (!devices) {
		usbi_err(ctx, "opendir devices failed errno=%d", errid);
		return LIBUSB_ERROR_IO;
	}
...
static int op_open(struct libusb_device_handle *handle)
{
	...
	if (hpriv->fd < 0) {
		...
		} else if (errid == ENOENT) {
			return LIBUSB_ERROR_NO_DEVICE;
		} else {
			usbi_err(HANDLE_CTX(handle),
				"open failed, code %d errno %d", hpriv->fd, errid);
			return LIBUSB_ERROR_IO;
		}
	}
...
static int op_set_configuration(struct libusb_device_handle *handle, int config)
{
	...
	if (r) {
		...
		else if (errid == ENODEV)
			return LIBUSB_ERROR_NO_DEVICE;

		usbi_err(HANDLE_CTX(handle), "failed, error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
...
static int op_claim_interface(struct libusb_device_handle *handle, int iface)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle),
			"claim interface failed, error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
	return 0;
}
...
static int op_release_interface(struct libusb_device_handle *handle, int iface)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle),
			"release interface failed, error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
	return 0;
}
	...
	int altsetting)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle),
			"setintf failed error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
	...
	unsigned char endpoint)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle),
			"clear_halt failed error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
...
static int op_reset_device(struct libusb_device_handle *handle)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle),
			"reset failed error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
	...
	int interface)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle),
			"get driver failed error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
	...
	int interface)
{
	...
	if (r) {
		if (errid == ENODATA)
			return LIBUSB_ERROR_NOT_FOUND;
		else if (errid == EINVAL)
			return LIBUSB_ERROR_INVALID_PARAM;
		else if (errid == ENODEV)
			return LIBUSB_ERROR_NO_DEVICE;

		usbi_err(HANDLE_CTX(handle),
			"detach failed error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	}
	...
	int interface)
{
	...
	if (r < 0) {
		...
		usbi_err(HANDLE_CTX(handle),
			"attach failed error %d errno %d", r, errid);
		return LIBUSB_ERROR_OTHER;
	} else if (r == 0) {
		return LIBUSB_ERROR_NOT_FOUND;
	}
		...
		if (r < 0) {
			...
			if (errid == ENODEV) {
				...
				usbi_err(TRANSFER_CTX(transfer),
					"submiturb failed error %d errno=%d", r, errid);
				r = LIBUSB_ERROR_IO;
			}
			...
			for (j = 0; j < i; j++) {
				...
				if (tmp && errid != EINVAL)
					usbi_warn(TRANSFER_CTX(transfer),
						"unrecognised discard errno %d", errid);
			}
...
static int cancel_control_transfer(struct usbi_transfer *itransfer)
{
	...
	if(r) {
		if (errid == EINVAL) {
			...
			usbi_err(TRANSFER_CTX(transfer),
				"unrecognised DISCARD code %d", errid);
			return LIBUSB_ERROR_OTHER;
		}
	}
