...
int usbi_io_init(struct libusb_context *ctx)
{
	...
	if (ctx->timerfd >= 0) {
		...
		usbi_dbg("timerfd not available (code %d error %d)", ctx->timerfd, errid);
		ctx->timerfd = -1;
	}
#endif
...
static int calculate_timeout(struct usbi_transfer *transfer)
{
	...
	if (r < 0) {
		usbi_err(ITRANSFER_CTX(transfer),
			"failed to read monotonic clock, errno=%d", errid);
		return r;
	}
...
API_EXPORTED int libusb_wait_for_event(libusb_context *ctx, struct timeval *tv)
{
	...
	if (r < 0) {
		usbi_err(ctx, "failed to read realtime clock, error %d", errid);
		return LIBUSB_ERROR_OTHER;
	}
...
static void handle_timeout(struct usbi_transfer *itransfer)
{
	...
	if (r < 0)
		usbi_warn(TRANSFER_CTX(transfer),
			"async cancel failed %d errno=%d", r, errid);
}
	...
	struct timeval *tv)
{
	...
	if (r < 0) {
		usbi_err(ctx, "failed to read monotonic clock, errno=%d", errid);
		return LIBUSB_ERROR_OTHER;
	}
